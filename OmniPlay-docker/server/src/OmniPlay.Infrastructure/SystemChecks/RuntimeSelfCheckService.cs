...
public sealed class RuntimeSelfCheckService : IRuntimeSelfCheckService
{
    ...
    private static RuntimeSelfCheckItem CheckFfmpeg(FfmpegTranscodeCapabilities capabilities)
    {
        ...
        var availableDetail = isSystemFfmpeg
            ? $"FFmpeg 可用：{capabilities.FfmpegPath}。当前使用容器内 FFmpeg；如硬解能力不完整，请使用包含对应 VAAPI/QSV/V4L2 支持的镜像或通过 OMNIPLAY_FFMPEG_PATH 指定完整 FFmpeg。"
    ...
    private static HardwareDeviceAccessCheck CheckFirstDeviceFile(string directory, string pattern)
    {
        ...
        if (paths.Count == 0)
        {
            return new HardwareDeviceAccessCheck(
                string.Empty,
                false,
                $"FFmpeg 暴露了 v4l2m2m 能力，但容器内未发现 {directory}/{pattern}。请确认宿主机视频设备已挂载到容器。");
        }
        ...
        return new HardwareDeviceAccessCheck(
            string.Join(",", paths),
            false,
            $"发现 {string.Join(", ", paths)}，但当前容器用户 {GetProcessUserName()} 无法访问。请在 compose 中挂载 /dev/dri 或对应视频设备，并确认容器用户具有 video/render 设备权限。");
    }
    ...
    private static HardwareDeviceAccessCheck CheckDeviceFile(string devicePath)
    {
        if (!File.Exists(devicePath))
        {
            return new HardwareDeviceAccessCheck(
                devicePath,
                false,
                $"容器内未发现 {devicePath}。请确认 compose.yml 已挂载宿主机硬解设备。");
        }
        ...
        catch (Exception ex) when (ex is IOException or UnauthorizedAccessException)
        {
            return new HardwareDeviceAccessCheck(
                devicePath,
                false,
                $"当前容器用户 {GetProcessUserName()} 无法访问 {devicePath}：{ex.Message}。请在 compose 中挂载 /dev/dri 或对应视频设备，并确认容器用户具有 video/render 设备权限。");
        }
    }
