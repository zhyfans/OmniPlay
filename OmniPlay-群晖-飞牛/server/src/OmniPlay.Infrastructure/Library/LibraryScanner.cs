...
public sealed class LibraryScanner : ILibraryScanner
{
    ...
    {
        var maxSize = files.Count == 0 ? 0 : files.Max(static file => file.FileSizeBytes ?? 0);
        if (maxSize <= 0)
        {
            return files.Take(1);
        }
        ...
        return files.Where(file => (file.FileSizeBytes ?? 0) >= threshold);
    }
        ...
        CancellationToken cancellationToken)
    {
        ...
        var episodeId = StableId.Create("episode", seasonId, episodeInfo.Episode.ToString());
        ...
        await ExecuteAsync(connection, transaction, """
            ...
            ("$title", $"{showTitle} {episodeInfo.DisplayName}"));
